...
import { AnswerEditValidator, AnswerValidator } from "@/validators/answer";
...
export async function DELETE(req: NextRequest, {params}:{params:{id: string, answerId: string}}) {
  ...
  export async function PATCH(req: NextRequest, {params}:{params:{id: string, answerId: string}}) {
    try {
      ...
      const updatedAnswer = await AnswerEditValidator.parse(updatedData);
      //update project
      await db.predefinedAnswer.update({
       where:{
            id: params.answerId,
            projectId:params.id,
        },
